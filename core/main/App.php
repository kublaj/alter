...
class App {
	...
	public function __construct(){
		...
		if(file_exists(ALTER_APP.'/config.json')){

			$userOptions = (array)json_decode(file_get_contents(ALTER_APP.'/config.json'));
			foreach($userOptions as $option => $value){

				if(in_array($option, $allowedUserOptions)){
					$this->{$option} = $value;
				};
	...
	public function registerController($controller){
		...
		$controllerName = str_replace('controller', '', strtolower(get_class($controller)));
